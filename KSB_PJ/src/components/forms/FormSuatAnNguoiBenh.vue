...
const rows = ref([
  { mucSo: 1, noiDung: 'Cơm', ghiChuNoiDung: '', cheDo1KhoiLuong: '', cheDo1Dat: null, cheDo2KhoiLuong: '', cheDo2Dat: null, ghiChu: '' },
  { mucSo: 2, noiDung: 'Món mặn', ghiChuNoiDung: '', cheDo1KhoiLuong: '', cheDo1Dat: null, cheDo2KhoiLuong: '', cheDo2Dat: null, ghiChu: '' },
  { mucSo: 3, noiDung: 'Món chay', ghiChuNoiDung: '', cheDo1KhoiLuong: '', cheDo1Dat: null, cheDo2KhoiLuong: '', cheDo2Dat: null, ghiChu: '' },
  { mucSo: 4, noiDung: 'Món xào/luộc', ghiChuNoiDung: '', cheDo1KhoiLuong: '', cheDo1Dat: null, cheDo2KhoiLuong: '', cheDo2Dat: null, ghiChu: '' },
  { mucSo: 5, noiDung: 'Món canh', ghiChuNoiDung: '', cheDo1KhoiLuong: '', cheDo1Dat: null, cheDo2KhoiLuong: '', cheDo2Dat: null, ghiChu: '' }
])

applyReportTemplate('SuatAnNguoiBenh', rows.value)

const tubeRows = ref([
  { mucSo: 1, noiDung: 'Súp xay', cheDo1KhoiLuong: '', cheDo1Dat: null, cheDo2KhoiLuong: '', cheDo2Dat: null },
  { mucSo: 2, noiDung: 'Sữa', cheDo1KhoiLuong: '', cheDo1Dat: null, cheDo2KhoiLuong: '', cheDo2Dat: null }
])

const requirementRows = ref([
  {
    mucSo: 1,
    noiDung: 'Món súp xay ăn qua ống thông',
    moTa: 'Món ăn lỏng, đồng nhất, ít lợn cợn, đảm bảo lưu thông qua ống thông dễ dàng.',
    dat: null,
    ghiChu: ''
  }
])
...
const submitForm = async () => {
  ...
  try {
    await saveReport({
      ...
      yeuCauRieng: requirementRows.value,
            ...
            <tbody>
              <tr v-for="row in rows" :key="row.mucSo">
                ...
                <td>
                  ...
                  <input v-model="row.ghiChuNoiDung" type="text" class="glass-input-sm" placeholder="Mô tả thêm nếu cần..." />
                </td>
                ...
                <td><input v-model="row.ghiChu" type="text" class="glass-input-sm" placeholder="Ghi chú..." /></td>
              </tr>
            </tbody>
          </table>
        </div>
      ...
      <div class="glass-card section-card">
        ...
        <div class="table-responsive">
          <table class="glass-table">
            ...
            <tbody>
              <tr v-for="row in tubeRows" :key="`tube-${row.mucSo}`">
                <td class="text-center">{{ row.mucSo }}</td>
        ...
        <div class="table-responsive">
          <table class="glass-table">
            ...
            <tbody>
              <tr v-for="row in requirementRows" :key="`req-${row.mucSo}`">
                <td class="text-center">{{ row.mucSo }}</td>
                <td>
                  <strong>{{ row.noiDung }}</strong>
                  <div class="muted">{{ row.moTa }}</div>
                </td>
                <td class="text-center"><input v-model="row.dat" type="radio" :name="`req_${row.mucSo}`" :value="true" /></td>
                <td class="text-center"><input v-model="row.dat" type="radio" :name="`req_${row.mucSo}`" :value="false" /></td>
                <td><input v-model="row.ghiChu" type="text" class="glass-input-sm" placeholder="Ghi chú..." /></td>
              </tr>
            </tbody>
          </table>
...
.form-row, .option-grid { display: grid; grid-template-columns: repeat(2, minmax(0, 1fr)); gap: 16px; }
