...
const completedCount = computed(() => rows.value.filter(item => item.dat !== null).length)
...
const scrollToFirstUnchecked = () => {
  ...
  const uncheckedItem = rows.value.find(item => item.dat === null)
  if (uncheckedItem) {
...
const submitForm = async () => {
  const unchecked = rows.value.find(item => item.dat === null)
  if (unchecked) {
        ...
        <div class="glass-card section-card">
          ...
          <div class="table-responsive">
            <table class="glass-table">
              ...
              <tbody>
                <tr v-for="row in rows" :key="row.mucSo" :id="'item-row-' + row.mucSo" :class="{ 'row-missing-highlight': highlightUnchecked && row.dat === null }">
